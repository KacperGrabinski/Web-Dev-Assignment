...
function displaySearch() {
  ...
  for (let i = 0; i < videosList.length; i++) {
    const itemDescription = videosList[i].description
    if (itemDescription.includes(keyWord)) {
      resultsEle.innerHTML += generateRowHTML(videosList[i])
      numResults++
    }
  }
  if (numResults == 0) {
    resultsEle.innerHTML = `No videos found with the keyword ${keyWord}`
  }
}
...
function uploadToList() {
  ...
  let descResult = desc.value


  let year = document.getElementById('txt-year')
  let yearResult = year.value


  let thumbnail = document.getElementById('txt-thumbnail')
  let thumbnailResult = thumbnail.value
...
document.getElementById('btn-search').addEventListener("click",displaySearch);
document.getElementById('btn-show-all').addEventListener("click",displayAll);